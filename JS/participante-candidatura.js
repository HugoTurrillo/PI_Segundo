/**
 * Muestro el estado de la candidatura del participante y el detalle en popup; escapo datos con escapeHtml.
 */
...
` : ""}
        ...
        <h4>Sinopsis</h4>
        <p class="candidatura-sinopsis">${esc(c.sinopsis)}</p>

        ${c.estado === "rechazada" ? `
  <div style="margin-top:1rem;">
    <p style="color:red;"><strong>Motivo rechazo</strong></p>
    <p>${esc(c.motivo_rechazo ?? "")}</p>

    <h4>Editar candidatura y subsanar</h4>

    <label>Nuevo título (opcional)</label>
    <input type="text" class="tituloEditado" value="${esc(c.titulo_obra)}">

    <label>Nueva sinopsis (opcional)</label>
    <textarea class="sinopsisEditada">${esc(c.sinopsis)}</textarea>
